...
double next()
{
	const double val = min + (max - min) * g3d_random();
	return val;
}

// Generate a g3d_random point on different distribution
void generateSinglePoint(REAL* x, REAL* y, REAL* z, Distribution dist)
{
	...
	switch (dist)
	{
		...
		case GaussianDistribution:
		{
			...
			do {
				do {
					x1 = 2.0 * g3d_random() - 1.0;
					x2 = 2.0 * g3d_random() - 1.0;
					x3 = 2.0 * g3d_random() - 1.0;
					w = x1 * x1 + x2 * x2 + x3 * x3;
				} while (w >= 1.0);
		...
		case SphereDistribution:
		{
			...
			//do 
			//{
			//	a = g3d_random() * 2.0 - 1.0;
			//	b = g3d_random() * 2.0 - 1.0;
			//	c = g3d_random() * 2.0 - 1.0;
			//	d = g3d_random() * 2.0 - 1.0;
...
// Generate g3d_random triangle set as input constraint face
	...
	int numoftri, int ** trilist, double minarea)
{
	...
	*trilist = new int[(numoftri+numofhull) * 3];
	...
	// Generate g3d_random triangle set
...
// Generate g3d_random dangling segment as input segment
	...
	int numofedge, int ** edgelist)
{
	...
	*edgelist = new int[numofedge * 2];
	...
	// Generate g3d_random dangling set
	...
	int seed, Distribution dist, double minarea)
{
	...
	// Initialize g3d_random number generator
...
bool generatePoints(int numofpoint, int* curnumofpoint, REAL * list, Distribution dist)
{
	...
	// Continue to generate g3d_random point set
	while (i < numofpoint)
	{
		// generate a g3d_random point
		generateSinglePoint(&x, &y, &z, dist);
	...
	Distribution dist, double minarea)
{
	...
	// Generate g3d_random points and rectangles
	...
	while (*curnumofrect < numoftri)
	{
		...
		// Generate three g3d_random points to define a plane where the rectangle is located
		while (i < 3)
		{
			generateSinglePoint(&x, &y, &z, dist);
			tetVertex vert(x, y, z);
